...
pub mod batch {
    ...
    fn evaluate_args_valid(
        ...
        if let Some(schema) = schema {
            if let Some(max) = schema.pointer("/properties/percent/maximum") {
                if let Some(percent) = args.get("percent").and_then(|p| p.as_f64()) {
                    if let Some(max_val) = max.as_f64() {
                        if percent > max_val {
                            return (
                                Outcome::Fail,
                                format!("percent {} exceeds maximum {}", percent, max_val),
                            );
                        }
                    }
                }
